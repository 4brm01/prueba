  ...
  async (req, res, next) => {
    try {
      ...
      try {
        ...
        logger.warn('Stored procedure failed, trying direct query', { error: spError.message, ip: req.ip });
      ...
      if (result.recordset.length === 0) {
        logger.warn('Usuario no encontrado', { username: username, ip: req.ip });
      ...
      if (!isPasswordValid) {
        logger.warn('Contraseña incorrecta', { username: username, ip: req.ip });
        const error = new Error('Invalid credentials');
        error.statusCode = 401;
        throw error;
      }
